...
subtitle: "Part 0: Syllabus Review & Introduction to `R`"
author: 
...
- Primary way to plot data in `R`. 

- `plot(x, y)` produces a scatterplot of the numbers in `x` vs. the numbers in `y`. 
...
```{r chunk1, echo=F,include = T, eval = F, cache = T, massage = T}
...
- Producing a *contour plot* in order to represent three-dimensional data,
...
- Working the same way as `contour()`, 
...
## An example (ISL, pp. 54)

- The `College` data set can be found on the book website (https://www.statlearning.com/s/College.csv).
...
We don't really want `R` to treat this as a variable.
Instead,
it is better to have `row.names` recording these names
...
- Use `summary()` function to produce a numerical summary of variables in the data set.
```{r chunk7, echo=F,include = T, eval = F, cache = T, massage = T}
summary(college)
```

- Use `pairs()` function to produce a scatterplot matrix of the first ten variables of the data.
```{r chunk8, echo=F,include = T, eval = F, cache = T, massage = T}
pairs(data.matrix(college[,1:10])) # data.matrix() converting a data frame to a numeric matrix
```

- Use `boxplot()` function to produce side-by-side boxplots of `Outstate` versus `Private`.
```{r chunk9, echo=F,include = T, eval = F, cache = T, massage = T}
boxplot(Outstate ~ Private, data = college)
```

- Create a new qualitative variable, called `Elite`,
by dividing universities into two groups based on whether or not the proportion
...
- Use `hist()` function to produce histograms with different numbers of bins for quantitative variables.
