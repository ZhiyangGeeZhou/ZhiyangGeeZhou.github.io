...
- Forward selection
  ...
  1. Select the best predictor: Add the predictor that improves the model the most 
...
maximal = lm(
  ln_wage~(age_sqrt+maritl+race+education+health)^2, 
  data=wage
)
...
### Penalization/regularization/shirinkage (ISL, Sec. 6.2)

- Model/variable selection 
$\Leftrightarrow$ compressing certain coefficients to zeros
...
- Automatic 
- Model-based: 
e.g., if using linear models, the model selection relies on the mean of response only.
