...
Group points and color them by a categorical variable (e.g., `gender`).
...
starwars %>% 
  ...
  labs(
    title = "Height vs. Mass",
    x = "Mass (kg)", y = "Height (cm)", color = "Gender"
  ) +
  theme_light()
```
...
starwars %>% 
  ...
  ggplot(
    aes(
      x = mass, y = height, 
      shape = gender,  # different symbols for different genders
  ...
  labs(
    title = "Height vs. Mass",
    x = "Mass (kg)", y = "Height (cm)", shape = "Gender", size = 'Mass'
  ) +
  theme_grey()
```
...
starwars %>% 
  ...
  geom_point() +
  labs(
    title = "Height vs. Mass by gender",
...
```{r, eval = F}
starwars %>% 
  ggplot(aes(x = fct_rev(species), fill = gender)) + # Reverses the order of species to correct flipping
...
ggplot(starwars, aes(x = '', fill = gender)) +
  geom_bar(width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Numbers of Characters by Gender", x='', y='') +
  theme_minimal()
```
...
starwars %>%
  ...
  coord_polar(theta = "y") + 
  labs(title = "Proportion of Characters by Gender", x='', y='') +
  theme_minimal()
```

### Box plot

Compare heights by gender.

```{r, eval = F}
starwars %>% 
  filter(!is.na(height)) %>%
  replace_na(list(gender = 'Unknown')) %>%
  ggplot(aes(x = gender, y = height, fill = gender)) + # "fill=gender": paint boxes by gender
  geom_boxplot() +
...
y <- x
f_matrix <- outer(x, y, function(x, y) x^2 + y^2)
df <- expand.grid(x = x, y = y) %>% mutate(f = as.vector(f_matrix))
